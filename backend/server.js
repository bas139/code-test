  ...
  socket.on("run_code", async (data) => {
    ...
    if (isCompiled) {
      ...
      compiler.on("error", (err) => {
        socket.emit(
          "output",
          `\x1b[31mFailed to start compiler: ${err.message}\x1b[0m\r\n`,
        );
        socket.emit("exit");
        setTimeout(() => {
          fs.rm(sessionDir, { recursive: true, force: true }, () => {});
...
app.post("/api/execute", async (req, res) => {
  ...
  if (isCompiled) {
    ...
    compiler.on("error", (err) => {
      ...
      res
        .status(500)
        .json({ error: `Failed to start compiler: ${err.message}` });
    });
  } else {
    runCode();
  }
...
app.post("/api/ai-hint", async (req, res) => {
  ...
  if (!process.env.GEMINI_API_KEY) {
    return res
      .status(500)
      .json({ error: "GEMINI_API_KEY is not configured in backend/.env" });
  }

  try {
    const ai = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY });
    let systemInstruction =
      "คุณเป็นผู้ช่วยโปรแกรมเมอร์ ให้บอกสั้นๆ กระชับที่สุดว่าโค้ดผิดตรงไหน (เช่น ลืมใส่ semicolon ที่บรรทัด ..., ตัวแปรผิดชื่อ) ตอบสั้นๆ ไม่เกิน 1-2 ประโยค ห้ามอธิบายยาวเยิ่นเย้อ ห้ามเฉลยโค้ดที่ถูกต้องเด็ดขาด";
    ...
    res
      .status(500)
      .json({ error: "Failed to generate AI hint", details: error.message });
  }
});
...
`;
    ...
    res
      .status(500)
      .json({ error: "Failed to generate AI trace", details: error.message });
  }
});
